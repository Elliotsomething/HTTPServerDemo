...
@interface YHSSZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end


@implementation YHSSZipArchive {
	NSString *_path;
	NSString *_filename;
    zipFile _zip;
}
...
{
	return [self createZipFileAtPath:path withFilesAtPaths:paths andExcludeFileNames:nil password:nil];
}

+ (BOOL)hasSubComponents:(NSArray *)components ofPath:(NSString *)path
{
	NSAssert([[NSSet setWithArray:components] count] == components.count, @"must be uniqueness");
	NSMutableSet *totolSet = [NSMutableSet setWithArray:[path componentsSeparatedByString:@"/"]];
	NSInteger totolCount = components.count + totolSet.count;
	[totolSet addObjectsFromArray:components];
	if (totolSet.count != totolCount) {
		return YES;
	}
	return NO;
}
+ (BOOL)createZipFileAtPath:(NSString *)path
		...
		andExcludeFileNames:(NSArray *)excludeFileNames
				   password:(NSString *)password
{
	BOOL success = NO;
	YHSSZipArchive *zipArchive = [[YHSSZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
			...
			if (isExist) {
				if (isDir) {
					...
					while ((fileName = [dirEnumerator nextObject])) {
						if ([self hasSubComponents:excludeFileNames ofPath:fileName]) {
							continue;
						}
						NSString *fixedFileName = fileName;
						if (! [dirName isEqualToString:@".."]) {
							fixedFileName = [dirName stringByAppendingPathComponent:fileName];
						}
						...
						if (!isDir) {
							[zipArchive writeFileAtPath:fullFilePath withFileName:fixedFileName andPassword:password];
						}
					}
				}else{
					[zipArchive writeFileAtPath:path withFileName:nil andPassword:password];
				}
...
{
	return [self writeFileAtPath:path withFileName:fileName andPassword:nil];
}
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName andPassword:(NSString *)password
{
    ...
    zipOpenNewFileInZipPW(_zip, afileName, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION, [password UTF8String], crc);
...
{
	return [self writeData:data filename:filename password:nil];
}
- (BOOL)writeData:(NSData *)data filename:(NSString *)filename password:(NSString *)password
{
    if (!_zip) {
		return NO;
    }
	...
	zipOpenNewFileInZipPW(_zip, [filename UTF8String], &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION, [password UTF8String], 0);
